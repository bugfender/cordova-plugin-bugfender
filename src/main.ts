...
class BugfenderClass {
    ...
    public setPrintToConsole(v) {
        checkLoaded();
        printToConsole = v;
    }

    public getPrintToConsole() {
        checkLoaded();
        return printToConsole;
    }

    public getDeviceUrl(callback) {
        checkLoaded();
        if (window['device'] && window['device'].platform != 'browser')
            window['cordova'].exec(callback, null, 'Bugfender', 'getDeviceUrl', []);
    }

    public getSessionUrl(callback) {
        checkLoaded();
        if (window['device'] && window['device'].platform != 'browser')
            window['cordova'].exec(callback, null, 'Bugfender', 'getSessionUrl', []);
    }

    public sendCrash(title, markdown, callback) {
        checkLoaded();
        if (window['device'] && window['device'].platform != 'browser')
            window['cordova'].exec(callback, null, 'Bugfender', 'sendCrash', [title, markdown]);
    }

    public sendUserFeedback(title, markdown, callback) {
        checkLoaded();
        if (window['device'] && window['device'].platform != 'browser')
            window['cordova'].exec(callback, null, 'Bugfender', 'sendUserFeedback', [title, markdown]);
    }

    public showUserFeedbackUI(title, hint, subjectHint, messageHint, sendButtonText, cancelButtonText, callback) {
        checkLoaded();
        if (window['device'] && window['device'].platform != 'browser')
...
var logWithLevel = function (level, args) {
    ...
    var tag = "";
    var message = format.apply(this, args);
